...
//                $content = isset($c) ? $c->innertext : '';
    ...
    public function indexed()
    {
        ...
        foreach ($col as $c) {
            ...
            //$d = array();
            foreach ($str_content as $word) {
                if ($word != '') {
                    //array_push($d, $doc_id);
                    $ind = $indexed->find(array('term_id' => $word));
                    $in = $indexed->findOne(array('term_id' => $word));

                    if (!empty($in)) { //print_r($in['doc_id']); die();
                        $data = array(
                            'doc_id' => $in['doc_id'] . ' ' . $doc_id
                        );
                        $indexed->update(array('term_id' => $word), array('$set' => $data));
                    } else {
                        $data = array(
                            'term_id' => $word,
    ...
    public function postSearch()
    {
        ...
        foreach ($data as $d) {
            ...
            echo '<p><strong>Created at:</strong>' . $d['created_at'] . '</p><hr/>';
        }
